...
Considerations should be made towards using point sources with no source time functions, % in discussion? 
...
\section{Discussion}
The long term continuous recordings of colocated rotation and translation waveforms for a large number of events currently provides an extensive catalog of observations with which it is possible to explore the potential uses of rotational signals for varying source parameters including magnitude, moment tensor, epicentral distance etc. The results of this work present only a preliminary address of this dataset, and hopes to open the door to the possibilities of large scale analyses on a very unique observable.

The results attained in this study were very interesting, as in they subverted the expectations set out at the beginning of the research. The main motivation for the initial study in Igel et al. 2007, was to determine whether or not the measured decay rate of rotation amplitudes could be explained by the currently accepted surface wave magnitude equation. In this study we expanded our question to how observed rotations decayed with distance compared to translations, and how this compares to the decay of observed translations for various components. In order to provide some form of comparison, in a situation where a similar set of observations was out of the question, we turned to waveform modelling; again whose results provided stark differences from previously gained information.

Addressing the results given in Table \ref{tab:scales}, it was very interesting to see how both rotation and rotation rate fell far above all measurements of velocity decay rates. In an ideal case, both rotation and rotation rate should only be sensitive to the transverse nature of the Love wave. Considerations should be given towards scattering of waves which provides a nonideal situation, however because the IASPEI scale is defined for the vertical component of Rayleigh waves, we see that it is a much better indication of Love wave sensitive rotations than either transverse velocity or vertical velocity. 

Comparisons with the IASPEI scale should be taken with caution however. This broadband scale was devised by Karnik et al and Vanek et al. as a global averaging of magnitudes. Because of this, all local and regional effects from these scales are included in the averaging, which would present a difference in setting as compared to the single station derivation that is performed in this study. However, regardless of the number of stations, the magnitude scale should be a generally applicable equation that only relates distance with peak amplitude as a function of magnitude. The large degree of variation between the IASPEI scale and vertical velocity found in this study have been addressed in previous publications; Herak \& Herak (1993) as well as Ambraseys-Free (1997) %CITE DATE?
