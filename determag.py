...
def event_info_data(event, station):
    ...
    return latter, lonter, depth, startev, rt, ac, baz, gcdist, net_r, net_s,\
        chan1, chan2, chan3, chan4, sta_r, sta_s, loc_r, loc_s
...
def process_save(ac,rt,baz,cutoff,station,is_local,min_lwi,max_lwf,
    ...
    *be careful with the copy statement
    ...
    plt.savefig('OUTPUT/WET/imgs/'+'WET_REV_'+tag_name+'.png',dpi=150)
    plt.savefig(folder_name+'WET_REV_'+tag_name+'.png',dpi=150)
    #plt.show() 
    plt.close()
...
def store_info_json(tag_name, folder_name, ac, rt, baz, peak2troughs, 
                    periods, zero_crossings_abs, station, startev, event,
                    net_r, net_s, chan1, chan2, chan3, chan4, sta_r, sta_s,
                    loc_r, loc_s, event_source, depth, magnitude):
    """
    ...
    dic = OrderedDict([
            ...
            ('peak_filtered_rotation_rate', rr_max),
            ('dominant_period_rotation_rate',rr_per),
            ('rotation_rate_zero_crossing',rr_zca),
            ('peak_filtered_rr_unit', 'nrad/s'),
            ('peak_filtered_rotation', rt_max),
            ('dominant_period_rotation',rt_per),
            ('rotation_zero_crossing',rt_zca),
            ('peak_filtered_rt_unit', 'nrad'),
...
catpath = './catalogs/WET_244_catalog.xml'
...
# manually enter event number to create catalog
#ID_list = ['5198775']
#cat = Catalog()
#c = fdsnClient(event_source)
#for ID in ID_list:
#    cat.extend(c.get_events(eventid=ID))
...
for event in cat[:1]:
    try:
        ...
        print '________________________________________________________________'
        latter,lonter,depth,startev,rt,ac,baz,gcdist,net_r,net_s,\
            chan1,chan2,chan3,chan4,sta_r,sta_s,loc_r,loc_s = \
        ...
        if not os.path.exists('OUTPUT/{}'.format(station)):
            print 'Creating station folder...'
            os.makedirs('OUTPUT/{}'.format(station))

        # i.e. 'OUTPUT/WET/GCMT_2011-03-11T
        ...
        print 'Saving data...'
        store_info_json(tag_name,folder_name,ac,rt,baz,peak2troughs,
                        periods,zero_crossings_abs,station,startev,event,
                        net_r,net_s,chan1,chan2,chan3,chan4,sta_r,sta_s,
